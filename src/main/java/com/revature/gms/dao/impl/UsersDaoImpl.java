...
public boolean checkByMailId(String mailId) {
	...
	try {
		...
		connection = DbUtils.getConnection();
		String sql = "select u.id,u.name,u.fathername,u.email,u.dateofjoining,u.active,u.activeaccount,u.roles,d.id,d.name from users as u, departments as d where u.department=d.id and active=1";
		preparedStatement = connection.prepareStatement(sql);
		resultSet = preparedStatement.executeQuery();
